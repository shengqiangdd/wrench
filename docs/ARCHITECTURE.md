...
**逃生口**：右上「画布」芯片（关 = 贴屏 1:1，持久化在 `wrench_ssh_canvas`）。
`tmux` / `top` / `nano` 这类"占整屏但走普通屏"的程序只会看到 12 行窗口（内容从窗口顶
对齐，状态栏可能被留在窗外），关掉芯片即回到与改造前完全一致的行为。
画布长到上限（80 行）而块还在长时，终端会提示一次「点右上 plain 可改成逐行日志」
（`isCanvasCappedOut`）—— 几何层到此为止，再往下只能靠程序侧的 plain。
