...
export const BUILTIN_COMMANDS: QuickCommand[] = [
  ...
  { id: 'builtin-sys-crontab', groupId: 'system', name: '定时任务列表', command: 'crontab -l 2>/dev/null || echo "无定时任务"', description: '当前用户的 cron 任务', isBuiltin: true },
  { id: 'builtin-sys-env', groupId: 'system', name: '环境变量', command: 'env | sort', description: '所有环境变量（排序后）', isBuiltin: true },
  { id: 'builtin-sys-lscpu', groupId: 'system', name: 'CPU 信息', command: 'lscpu', description: 'CPU 架构、核心数、频率', isBuiltin: true },
  { id: 'builtin-sys-lspci', groupId: 'system', name: 'PCI 设备', command: 'lspci 2>/dev/null | head -20 || echo "lspci 不可用"', description: '硬件设备列表', isBuiltin: true },
  ...
  { id: 'builtin-dk-logs', groupId: 'docker', name: '容器日志', command: 'docker logs --tail 50 {{container}}', description: '查看指定容器最新日志', isBuiltin: true,
    variables: [{ name: 'container', label: '容器名称/ID', placeholder: '例如: nginx' }] },
  { id: 'builtin-dk-exec', groupId: 'docker', name: '容器内执行命令', command: 'docker exec -it {{container}} {{command}}', description: '在容器内执行一条命令', isBuiltin: true,
    variables: [
      { name: 'container', label: '容器名称', placeholder: 'myapp' },
      { name: 'command', label: '命令', defaultValue: 'sh', placeholder: 'bash' },
    ] },
  { id: 'builtin-dk-inspect', groupId: 'docker', name: '容器详情', command: 'docker inspect {{container}} | head -60', description: '查看容器完整配置', isBuiltin: true,
  ...
  { id: 'builtin-net-dns', groupId: 'network', name: 'DNS 解析', command: 'nslookup {{domain}} 2>&1', description: 'DNS 解析验证', isBuiltin: true,
    variables: [{ name: 'domain', label: '域名', defaultValue: 'google.com', placeholder: 'example.com' }] },
  { id: 'builtin-net-ping', groupId: 'network', name: 'Ping 测试', command: 'ping -c 5 {{host}}', description: '网络延迟和丢包检测', isBuiltin: true,
    variables: [{ name: 'host', label: '目标主机', defaultValue: '8.8.8.8', placeholder: 'IP 或域名' }] },
  { id: 'builtin-net-traceroute', groupId: 'network', name: '路由追踪', command: 'traceroute {{host}} 2>&1 || echo "需安装 traceroute"', description: '网络路径追踪', isBuiltin: true,
    variables: [{ name: 'host', label: '目标', defaultValue: '8.8.8.8' }] },
  { id: 'builtin-net-iptables', groupId: 'network', name: '防火墙规则', command: 'iptables -L -n -v --line-numbers 2>/dev/null || echo "需 root 权限"', description: '当前 iptables 规则', isBuiltin: true },
  { id: 'builtin-net-port-check', groupId: 'network', name: '端口连通性', command: 'nc -zv {{host}} {{port}} 2>&1 || echo "nc 不可用，尝试 curl"; curl -s -m 3 telnet://{{host}}:{{port}} 2>&1 || echo "端口 {{port}} 不可达"', description: '检测远程端口是否开放', isBuiltin: true,
    variables: [
  ...
  { id: 'builtin-disk-find-big', groupId: 'disk', name: '查找大文件', command: 'find {{path}} -type f -size +{{size}} -exec ls -lh {} \\; 2>/dev/null | sort -k5 -rh | head -20', description: '在指定目录查找超过指定大小的文件', isBuiltin: true,
    variables: [
      { name: 'path', label: '搜索目录', defaultValue: '/', placeholder: '/var/log' },
      { name: 'size', label: '最小大小', defaultValue: '100M', placeholder: '50M' },
    ] },
  { id: 'builtin-disk-inode', groupId: 'disk', name: 'inode 使用率', command: 'df -i', description: '各分区 inode 使用情况', isBuiltin: true },
  ...
  { id: 'builtin-log-tailf', groupId: 'log', name: '实时跟踪日志', command: 'tail -f {{logfile}}', description: '跟踪指定日志文件的新增内容', isBuiltin: true,
    variables: [{ name: 'logfile', label: '日志路径', defaultValue: '/var/log/syslog', placeholder: '/var/log/nginx/access.log' }] },
  { id: 'builtin-log-grep', groupId: 'log', name: '搜索日志内容', command: 'grep -i "{{pattern}}" {{logfile}} 2>/dev/null | tail -30', description: '在日志文件中搜索关键词', isBuiltin: true,
    variables: [
      { name: 'pattern', label: '搜索关键词', placeholder: 'error' },
      { name: 'logfile', label: '日志文件', defaultValue: '/var/log/syslog', placeholder: '/var/log/nginx/error.log' },
    ] },

  // ═══ 服务管理 ═══
  { id: 'builtin-svc-list', groupId: 'service', name: '服务列表', command: 'systemctl list-units --type=service --state=running --no-pager 2>/dev/null | head -30 || service --status-all 2>/dev/null || echo "systemctl 不可用"', description: '正在运行的系统服务', isBuiltin: true },
  { id: 'builtin-svc-status', groupId: 'service', name: '服务状态', command: 'systemctl status {{service}} --no-pager -l 2>&1 | head -30', description: '查看服务状态和最近日志', isBuiltin: true,
    variables: [{ name: 'service', label: '服务名称', placeholder: 'nginx' }] },
  { id: 'builtin-svc-restart', groupId: 'service', name: '重启服务', command: 'sudo systemctl restart {{service}} && echo "已重启 {{service}}" || echo "重启失败"', description: '重启指定系统服务', isBuiltin: true,
    variables: [{ name: 'service', label: '服务名称', placeholder: 'nginx' }] },
  { id: 'builtin-svc-journal', groupId: 'service', name: '服务日志', command: 'journalctl -u {{service}} -n 50 --no-pager 2>&1', description: '查看指定服务的 systemd 日志', isBuiltin: true,
    variables: [{ name: 'service', label: '服务名称', placeholder: 'nginx' }] },

  // ═══ 安全 ═══
  { id: 'builtin-sec-last-logins', groupId: 'security', name: '最近登录', command: 'last -20 2>/dev/null || echo "last 命令不可用"', description: '最近登录用户的记录', isBuiltin: true },
  { id: 'builtin-sec-failed-logins', groupId: 'security', name: '失败登录统计', command: 'lastb 2>/dev/null | wc -l; echo "次失败尝试（总记录）"; echo "---最近---"; lastb 2>/dev/null | head -10', description: 'SSH 暴力破解检查', isBuiltin: true },
  { id: 'builtin-sec-open-ports', groupId: 'security', name: '开放端口和服务', command: 'ss -tlnp 2>/dev/null | tail -n +2 | awk \'{print $4, $6}\' | sort', description: '检查监听端口及其关联进程', isBuiltin: true },
  { id: 'builtin-sec-suspicious', groupId: 'security', name: '可疑进程检查', command: 'ps aux | grep -iE "(mine|crypt|miner|xmr|stratum)" | grep -v grep || echo "未发现可疑进程"', description: '扫描常见挖矿/恶意进程', isBuiltin: true },
  { id: 'builtin-sec-file-perm', groupId: 'security', name: '检查敏感文件权限', command: 'ls -la /etc/shadow /etc/passwd /root/.ssh 2>/dev/null', description: '检查关键系统文件权限', isBuiltin: true },
]
