/**
 * markdown → HTML（纯函数，无 React 依赖）
 *
 * 从 `components/MarkdownPreview.tsx` 抽出来：组件文件只应导出组件
 * （ESLint `react-refresh/only-export-components`），而这两个函数需要被单测直接调用。
 * 安全边界都在这里：HTML 转义 + 链接/图片目标白名单（见 `safeUrl`）。
 */
...
/**
 * markdown → HTML（导出以便单测直接验证转义/链接白名单，不必渲染 DOM）
 */
...
/**
 * 只允许安全的链接目标：http(s) / mailto / 站内相对路径与锚点。
 *
 * 与 `modules/ssh/AiSidebar.tsx` 的渲染器共用（那里的输出是模型生成的，同样不可信）。
 *
 * 为什么要挡：markdown 来源并不完全可信（AI 回复、日志内容、项目文件、剪贴板），
 * 而 `[点我](javascript:alert(1))` 会被渲染成「可点击」的
 * `<a href="javascript:...">` —— 点一下就在本页执行脚本，而本页存着会话令牌、
 * 并握着已连接的 SSH 会话。`data:` / `vbscript:` 同理，一律降级为 `#`。
 */
export function safeUrl(raw: string, opts: { allowImageData?: boolean } = {}): string {
  const url = raw.trim()
  if (/^(https?:|mailto:)/i.test(url)) return url
  if (url.startsWith('/') || url.startsWith('#')) return url
  // 图片单独允许 data:image/*（贴图场景）；链接不允许 data:（data:text/html 可执行脚本）
  if (opts.allowImageData && /^data:image\//i.test(url)) return url
  return '#'
}

/** 处理行内格式：粗体、斜体、删除线、链接、图片（导出以便单测） */
  ...
  return (
    text
      ...
      .replace(
        /!\[([^\]]*)\]\(([^)]+)\)/g,
        (_, alt, url) =>
          `<img src="${safeUrl(url, { allowImageData: true })}" alt="${alt}" class="cm-image" loading="lazy">`,
      )
      // 链接 [text](url)
      .replace(
        /\[([^\]]+)\]\(([^)]+)\)/g,
        (_, label, url) =>
          `<a href="${safeUrl(url)}" class="cm-link" target="_blank" rel="noopener noreferrer">${label}</a>`,
      )
      // 删除线 ~~text~~
