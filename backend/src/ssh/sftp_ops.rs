...
pub async fn disk_usage(session: &Arc<SshSession>, path: &str) -> Result<DiskUsage, String> {
    let cmd = format!("du -sb {} 2>/dev/null | head -1", shell_escape(path));
    ...
    let total_size: i64 = line.split_whitespace().next().and_then(|s| s.parse().ok()).unwrap_or(0);

    // Count files and dirs
    let count_cmd = format!("find {} -maxdepth 1 2>/dev/null | wc -l", shell_escape(path));
    let count_result = crate::ssh::executor::execute_command(session, &count_cmd)
        .await
        .map_err(|e| format!("count failed: {}", e))?;
    let total_entries: i64 = count_result.stdout.trim().parse().unwrap_or(0);
    let dir_count_cmd = format!("find {} -maxdepth 1 -type d 2>/dev/null | wc -l", shell_escape(path));
    let dir_result = crate::ssh::executor::execute_command(session, &dir_count_cmd)
        .await
    ...
    let largest_result = crate::ssh::executor::execute_command(session, &largest_cmd)
        .await
    ...
    if let Some(sudo_pwd) = sudo_password {
        ...
        match result {
            Ok(r) => {
                if r.exit_code == 0 {
                    ...
                    for p in paths {
                        failed.push(BatchDeleteError { path: p.clone(), error: r.stderr.trim().to_string() });
                    }
                }
            }
            Err(e) => {
                for p in paths {
                    failed.push(BatchDeleteError { path: p.clone(), error: e.clone() });
                }
            }
        }
    } else {
        ...
        for p in paths {
            ...
            match crate::ssh::executor::execute_command(session, &cmd).await {
                ...
                Ok(r) => failed.push(BatchDeleteError { path: p.clone(), error: r.stderr.trim().to_string() }),
                Err(e) => failed.push(BatchDeleteError { path: p.clone(), error: e.clone() }),
            }
        }
    }
